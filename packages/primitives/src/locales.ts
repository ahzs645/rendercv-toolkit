...
type LocaleTableEntry = {
  monthNames: string[];
  /** The value RenderCV ships for this language, used to detect user overrides. */
  present: string;
  /**
   * What the flattened position suffix shows. Identical to `present` except for
   * English, which has always rendered "Present" here and keeps doing so.
   */
  presentDisplay: string;
};

const LOCALE_TABLE: Record<string, LocaleTableEntry> = {
  "english": {
...
export const ENGLISH_DATE_LOCALE: DateLocale = {
  language: 'english',
...
function readStringList(value: unknown, expectedLength: number) {
 ...
 * is still honoured.
 */
export function resolveDateLocale(localeYaml: string | undefined | null): DateLocale {
  if (!localeYaml?.trim()) {
  ...
  const language =
    typeof locale.language === 'string' && locale.language.trim()
 ...
 * still round-trip.
 */
export function monthNumbersByName(locale: DateLocale): Record<string, string> {
