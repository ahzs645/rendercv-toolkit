...
import type { RenderCvRenderRequest, RenderCvRenderResult, RenderCvRenderer } from './index';
...
function prepareCliYaml(yaml: string): string {
  const document = YAML.parse(yaml, { maxAliasCount: 50 });
  if (!isRecord(document) || !isRecord(document.cv) || !isRecord(document.cv.sections)) return yaml;
  for (const entries of Object.values(document.cv.sections)) {
    if (!Array.isArray(entries)) continue;
    for (const entry of entries) {
      if (!isRecord(entry) || typeof entry.position !== 'string') continue;
      if (!entry.company && entry.position.startsWith('RCVSPACING')) entry.company = 'RCVCONTINUATION';
    }
  }
  return YAML.stringify(document);
}

async function stageTheme(yaml: string, directory: string, themesRoot: string) {
  const document = YAML.parse(yaml, { maxAliasCount: 50 });
  ...
  if (!theme || !/^[a-zA-Z0-9_-]{1,80}$/.test(theme)) return;
  ...
  if (theme !== 'ahmadstyle' && theme !== 'tylerstyle') return;
  for (const relative of ['ExperienceEntry.j2.typ', 'entries/ExperienceEntry.j2.typ']) {
    ...
    await writeFile(path, template.replace(
      '{% if entry.company %}',
      '{% if entry.company and entry.company != "RCVCONTINUATION" %}'
    ), 'utf-8');
  }
}
...
async function readBoundedOutput(stream: ReadableStream<Uint8Array>): Promise<string> {
  ...
  while (true) {
    ...
    if (done) break;
    if (!value || kept >= MAX_LOG_BYTES) continue;
...
export function createRenderCvCliRenderer(options: RenderCvCliRendererOptions): RenderCvRenderer {
  return {
    id: 'rendercv-cli',
    async render(request: RenderCvRenderRequest): Promise<RenderCvRenderResult> {
      ...
      try {
        ...
        try {
          ...
          if (timedOut) throw new Error(`RenderCV exceeded the ${timeoutMs / 1000}-second timeout.`);
          if (exitCode !== 0) {
        ...
        if (pdf.byteLength > maxPdfBytes) throw new Error(`Rendered PDF exceeds the ${maxPdfBytes}-byte limit.`);
        return {
          pdf,
...
export async function getRenderCvCliStatus(options: Pick<RenderCvCliRendererOptions, 'binary' | 'themesRoot'>) {
  return {
