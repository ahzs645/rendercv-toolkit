...
var DEFAULT_MAX_PDF_BYTES = 15 * 1024 * 1024;
...
function prepareCliYaml(yaml) {
  const document = import_yaml.default.parse(yaml, { maxAliasCount: 50 });
  ...
  return import_yaml.default.stringify(document);
}
...
function createRenderCvCliRenderer(options) {
  return {
    id: "rendercv-cli",
    async render(request) {
      ...
      try {
        ...
        await writeFile(inputPath, effectiveYaml, { encoding: "utf-8", mode: 384 });
        ...
        try {
          ...
          if (timedOut) throw new Error(`RenderCV exceeded the ${timeoutMs / 1e3}-second timeout.`);
          if (exitCode !== 0) {
            const details = (stderr || stdout).trim().slice(-4e3);
...
async function getRenderCvCliStatus(options) {
  return {
...
export {
  createRenderCvCliRenderer,
  getRenderCvCliStatus
};
